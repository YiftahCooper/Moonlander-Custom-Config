...
> **Status: Implemented and activated.** CopyQ owns F13/F19/F22, the transaction path is non-blocking, rapid-repeat callbacks are generation-guarded, and the original clipboard is restored only when it is still safe to do so.
...
**Architecture:** `runTransaction()` pastes synchronously and delegates delayed reselection and restoration to an injected scheduler backed by CopyQ's `afterMilliseconds()`. A state object stored on CopyQ's documented shared `global` object assigns monotonically increasing generations so stale callbacks cannot affect a newer transaction.
...
- [x] **Step 1: Replace fake sleeps with a controllable scheduler**
...
- [x] **Step 2: Add tests for the critical path and callbacks**
...
- [x] **Step 3: Add rapid-repeat and error tests**
...
- [x] **Step 4: Run the transaction tests and confirm RED**
...
Red-stage result recorded during implementation: failures because `runTransaction()` still called `sleep()` and the adapter had no asynchronous scheduling implementation.
...
- [x] **Step 1: Select or preserve the original snapshot**
...
- [x] **Step 2: Paste immediately and restore monitoring**
...
- [x] **Step 3: Schedule guarded follow-up work**
...
- [x] **Step 5: Back the adapter with CopyQ APIs**
...
- [x] **Step 6: Run the transaction tests and confirm GREEN**
...
- [x] **Step 1: Document the non-blocking behavior**
...
- [x] **Step 4: Reinstall the runtime scripts**
