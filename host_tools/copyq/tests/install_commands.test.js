...
const desired = [
  { name: 'Moonlander: Smart Title Case', globalShortcuts: ['F13'] },
  { name: 'Moonlander: Cycle Case', globalShortcuts: ['F19'] },
  { name: 'Moonlander: Transplant Hebrew-English', globalShortcuts: ['F22'] },
];

test('staging replaces only exact Moonlander names and removes shortcuts', () => {
  const unrelated = { name: 'Keep me', globalShortcuts: ['Ctrl+K'] };
  const oldMoonlander = { name: desired[0].name, globalShortcuts: ['F14'], old: true };
  ...
  assert.deepEqual(
    result.filter((command) => command.name.startsWith('Moonlander:')).map((command) => command.globalShortcuts),
    [[], [], []],
  );
  assert.equal(result.some((command) => Object.hasOwn(command, 'globalShortcut')), false);
});

test('activation preserves the requested F13 F19 F22 shortcuts', () => {
  const result = mergeCommands([], desired, true);
  assert.deepEqual(result.map((command) => command.globalShortcuts), [['F13'], ['F19'], ['F22']]);
  assert.equal(result.every((command) => command.isGlobalShortcut), true);
});

test('activation rejects a global shortcut owned by an unrelated command', () => {
  assert.throws(
    () => mergeCommands([{ name: 'Unrelated', globalShortcuts: ['f19'] }], desired, true),
    /F19.*Unrelated/i,
  );
});

test('similarly prefixed unrelated commands are preserved', () => {
  const nearMatch = { name: 'Moonlander: Cycle Case (old)', globalShortcuts: [] };
...
test('installation never calls importCommands', () => {
  ...
  let installed = [{ name: 'Keep me', globalShortcuts: [] }];
...
test('installation fails if CopyQ drops requested global shortcuts', () => {
  const previous = {
    commands: global.commands,
    setCommands: global.setCommands,
  };
  ...
  try {
    assert.throws(
      () => install('C:/Tools/Moonlander', 'C:/Tools/Moonlander/Reselect.exe', true),
      /shortcut verification failed/i,
    );
  } finally {
    global.commands = previous.commands;
    global.setCommands = previous.setCommands;
